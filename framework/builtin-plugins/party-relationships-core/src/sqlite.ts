export type PartyRelationshipsCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildPartyRelationshipsCoreSqliteMigrationSql(options: PartyRelationshipsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "party_relationships_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getPartyRelationshipsCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getPartyRelationshipsCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildPartyRelationshipsCoreSqliteRollbackSql(options: PartyRelationshipsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "party_relationships_core_");
  return [
...
export function getPartyRelationshipsCoreSqliteLookupIndexName(tablePrefix = "party_relationships_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getPartyRelationshipsCoreSqliteStatusIndexName(tablePrefix = "party_relationships_core_"): string {
