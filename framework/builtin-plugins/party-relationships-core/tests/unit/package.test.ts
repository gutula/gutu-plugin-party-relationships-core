...
import { businessFlowDefinitions, createPartyRecord, mergePartyRecords, activatePartyRole, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("party-relationships-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("party.parties.create");
    expect(manifest.publicEvents).toContain("party.created.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof createPartyRecord).toBe("function");
    expect(typeof mergePartyRecords).toBe("function");
    expect(typeof activatePartyRole).toBe("function");
