...
describe("party-relationships-core lifecycle integration", () => {
  const previousStateDir = process.env.GUTU_STATE_DIR;
  let stateDir = "";

  beforeEach(() => {
    stateDir = mkdtempSync(join(tmpdir(), "party-relationships-core-"));
  ...
  it("creates, advances, and reconciles a governed business record", async () => {
    const created = await createPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "party-relationships-core:demo",
      title: "Party & Relationships Core Demo",
      ...
      correlationId: "party-relationships-core:corr",
      processId: "party-onboarding:demo"
    });
    ...
    const reconciled = await reconcilePrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "party-relationships-core:demo",
      exceptionId: "party-relationships-core:exception",
    ...
    const amended = await amendPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "party-relationships-core:demo",
      amendedRecordId: "party-relationships-core:demo:amended",
      expectedRevisionNo: 5,
      title: "Party & Relationships Core Demo Amendment",
      reasonCode: "commercial-correction"
    });
    expect(amended.amendedRecordId).toBe("party-relationships-core:demo:amended");

    const reversed = await reversePrimaryRecord({
      tenantId: "tenant_demo",
      actorId: "actor_admin",
      recordId: "party-relationships-core:demo:amended",
      reversalRecordId: "party-relationships-core:demo:amended:reversal",
      expectedRevisionNo: 1,
      reasonCode: "cancel-amendment"
    });
    expect(reversed.reversalRecordId).toBe("party-relationships-core:demo:amended:reversal");
